  ...
  return (
    <div className="w-full bg-white rounded-2xl border border-slate-200/80 shadow-premium hover:shadow-premium-hover transition-all duration-500 overflow-hidden">
      ...
      <div className="sm:hidden divide-y divide-slate-100">
        <div className="bg-blue-600/[0.06] py-3 px-4 font-bold text-blue-700 text-xs tracking-wider uppercase border-l-[3px] border-l-blue-500 flex items-center gap-2">
        ...
        {coursesData.map((course) => (
          <div key={course.id} className="p-4 space-y-3 bg-white">
            <div className="flex items-start justify-between gap-2">
              ...
              {course.popular && (
                <span className="shrink-0 inline-flex items-center gap-0.5 px-2.5 py-1 rounded-full text-xs font-bold text-white bg-blue-600 shadow-sm">
            ...
            <Link
              ...
              className={`w-full min-h-12 py-3 px-4 rounded-xl text-xs font-bold tracking-wide uppercase transition-all duration-300 flex items-center justify-center gap-2 shadow-sm ${
                course.popular
                  ? "btn-premium-primary"
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-950 text-white text-xs sm:text-xs font-bold uppercase tracking-wider border-b border-slate-800">
          ...
          <tbody className="divide-y divide-slate-100 text-xs sm:text-sm">
            ...
            <tr className="bg-blue-600/[0.06]">
              <td colSpan={4} className="py-3 px-5 font-bold text-blue-700 text-xs sm:text-xs tracking-widest uppercase border-l-[3px] border-l-blue-500">
              ...
              >
                ...
                <td className="py-3.5 px-5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-bold text-slate-900 text-xs sm:text-sm group-hover:text-blue-600 smooth-transition">
                      {course.title}
                    </span>
                    {course.popular && (
                      <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-xs font-bold text-white bg-blue-600 shadow-sm">
                        <Sparkles size={7} /> HOT
                      </span>
                    )}
                  </div>
                ...
                {/* Duration */}
                <td className="py-3.5 px-4">
                  <span className="text-slate-600 font-bold text-xs sm:text-xs bg-slate-100 border border-slate-200/40 px-2.5 py-1 rounded-lg">
                    {course.duration}
                  </span>
                </td>
                ...
                <td className="py-3.5 px-4 text-center">
                  <Link
                    href={`/lien-he?select=${course.id}`}
                    className={`inline-flex items-center gap-1 px-3.5 py-2 rounded-full text-xs font-bold tracking-wide uppercase transition-all duration-300 ${
                      course.popular
                        ? "btn-premium-primary"
                        : "btn-premium-secondary"
                    }`}
