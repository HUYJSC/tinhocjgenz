...
export default function AdminAuditLogsPage() {
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      ...
      case "ROLE_CHANGE":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-rose-500/20 text-rose-300 border border-rose-500/30">
            <KeyRound size={11} /> Cấp Quyền / Vai Trò
          </span>
        );
      case "LOGIN_FAILED":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-amber-500/20 text-amber-300 border border-amber-500/30">
      ...
      case "LOGIN_SUCCESS":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
            <CheckCircle2 size={11} /> Đăng Nhập Thành Công
          </span>
        );
      case "LOGOUT":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-slate-500/20 text-slate-300 border border-slate-500/30">
            <LogOut size={11} /> Đăng Xuất
          </span>
        );
      case "SECURITY_ALERT":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-red-600/30 text-red-300 border border-red-500/50">
            <BellRing size={11} /> Cảnh Báo Bảo Mật
          </span>
        );
      case "ACCOUNT_LOCK":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-red-500/20 text-red-300 border border-red-500/30">
            <Lock size={11} /> Khóa Tài Khoản
          </span>
        );
      case "ACCOUNT_UNLOCK":
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold uppercase bg-blue-500/20 text-blue-300 border border-blue-500/30">
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      case "CRITICAL":
        return <span className="px-2 py-0.5 rounded text-xs font-bold uppercase bg-rose-950 text-rose-400 border border-rose-800">Critical</span>;
      case "WARNING":
        return <span className="px-2 py-0.5 rounded text-xs font-bold uppercase bg-amber-950 text-amber-400 border border-amber-800">Warning</span>;
      default:
        return <span className="px-2 py-0.5 rounded text-xs font-bold uppercase bg-blue-950 text-blue-400 border border-blue-800">Info</span>;
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
        <form onSubmit={handleSearch} className="relative w-full">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 rounded-xl bg-slate-950/70 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </form>

        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400">Hành động:</span>
            <select
              value={actionFilter}
              onChange={(e) => {
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400">Mức độ:</span>
            <select
          ...
          <button
            ...
            className="h-9 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-colors ml-auto cursor-pointer"
          >
            Lọc Dữ Liệu
          </button>
        </div>
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1 font-mono text-slate-300 text-xs">
                          <Globe size={11} className="text-slate-500" />
                          {log.ipAddress}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 max-w-xs text-slate-300 text-xs leading-relaxed">
                        {log.details}
                      </td>
