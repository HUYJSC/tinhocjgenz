...
import { Sparkles, Rss, FileText, Clock, CheckCircle2, Copy, TrendingUp, Brain, ArrowRight, RefreshCw, ExternalLink, ShieldCheck, Edit } from "lucide-react";
...
export default function ContentEngineOverviewPage() {
  ...
  const getStatusBadge = (status: Article["status"]) => {
    switch (status) {
      case "PUBLISHED":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-100 text-emerald-800">ĐÃ ĐĂNG</span>;
      case "APPROVED":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-blue-100 text-blue-800">ĐÃ DUYỆT</span>;
      case "AI_DRAFT":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-blue-100 text-blue-800">AI DRAFT</span>;
      case "REVIEW":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-amber-100 text-amber-800">CHỜ DUYỆT</span>;
      case "REJECTED":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-slate-200 text-slate-700">TỪ CHỐI</span>;
      case "DUPLICATE":
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-blue-100 text-blue-800">TRÙNG</span>;
      default:
        return <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-slate-100 text-slate-800">{status}</span>;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Lấy Hôm Nay</span>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-slate-900">{metrics?.fetchedToday ?? 0}</span>
            <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">bài</span>
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Từ các nguồn kích hoạt</p>
        </div>

        {/* Metric 2 */}
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-blue-600">AI Draft</span>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-blue-600">{metrics?.aiDraftCount ?? 0}</span>
            <Sparkles size={16} className="text-blue-400" />
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Điểm AI ≥ 80 / 100</p>
        </div>

        {/* Metric 3 */}
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-amber-600">Chờ Duyệt</span>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-amber-600">{metrics?.reviewCount ?? 0}</span>
            <Clock size={16} className="text-amber-400" />
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Cần Admin xem xét</p>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-emerald-600">{metrics?.publishedCount ?? 0}</span>
            <CheckCircle2 size={16} className="text-emerald-500" />
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Đang hiển thị trên web</p>
        </div>

        {/* Metric 5 */}
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-blue-600">Bài Bị Trùng</span>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-blue-600">{metrics?.duplicateCount ?? 0}</span>
            <Copy size={16} className="text-blue-400" />
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Lọc tự động &gt;85%</p>
        </div>

        {/* Metric 6 */}
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Điểm AI TB</span>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-bold text-blue-600">{metrics?.avgAiScore ?? 0}</span>
            <TrendingUp size={16} className="text-blue-500" />
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Độ phù hợp Tin học GenZ</p>
        </div>
      </div>
                ...
                <div className="min-w-0 pr-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    {src.priority === "OFFICIAL" && (
                      <span className="text-xs font-extrabold bg-blue-100 text-blue-800 px-1.5 py-0.2 rounded">
                        OFFICIAL
                      </span>
                    )}
                  </div>
