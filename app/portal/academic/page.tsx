...
import { BookOpen, AlertTriangle, Award, Plus, CheckCircle2, Layers } from "lucide-react";
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5 text-center min-w-[90px]">
              ...
              <div className="text-xs text-slate-400 font-bold">Lớp đang mở</div>
            </div>
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5 text-center min-w-[90px]">
              <div className="text-xl font-bold text-blue-400">142</div>
              <div className="text-xs text-slate-400 font-bold">Học viên active</div>
            </div>
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5 text-center min-w-[90px]">
              <div className="text-xl font-bold text-blue-400">99.4%</div>
              <div className="text-xs text-slate-400 font-bold">Tỷ lệ đỗ IIG</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2 overflow-x-auto">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 shrink-0 ${
            activeTab === "batches" ? "bg-blue-600 text-white shadow-md shadow-blue-600/30" : "bg-slate-800 text-slate-300 hover:bg-slate-700"
          }`}
        >
          <Layers size={14} />
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 shrink-0 ${
            activeTab === "certificates" ? "bg-blue-600 text-white shadow-md shadow-blue-600/30" : "bg-slate-800 text-slate-300 hover:bg-slate-700"
          }`}
        >
          <Award size={14} />
      ...
      {activeTab === "batches" && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-bold text-white">Danh Sách Lớp Đang Khai Giảng & Xếp Phòng</h3>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {batches.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="p-3 text-blue-400">{b.teacher}</td>
      ...
      {activeTab === "warnings" && (
        <div className="space-y-4">
          <h3 className="text-base font-bold text-white">Cảnh Báo Học Vụ Tự Động ({warnings.length})</h3>
          <div className="space-y-3">
            {warnings.map((w) => (
              <div key={w.id} className="p-5 rounded-2xl bg-slate-900 border border-blue-900/60 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-white text-sm">{w.studentName}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold uppercase bg-red-950 text-red-300 border border-red-800">
                      {w.riskLevel}
                    </span>
                  </div>
                  <p className="text-xs text-blue-400 font-semibold">{w.course} • Tỷ lệ vắng: {w.absentRate}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold cursor-pointer"
                  >
      ...
      {activeTab === "certificates" && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-base font-bold text-white">Duyệt & Phát Hành Chứng Chỉ Tốt Nghiệp</h3>
